...
module.exports = {
    async before(m) {
        ...
        if (!isSurrender) {
            let index = room.jawaban.indexOf(text)
            if (index < 0) {
                if (Math.max(...room.jawaban.filter((_, index) => !room.terjawab[index]).map(jawaban => similarity(jawaban, text))) >= threshold) m.reply('Un poco más!')
                return !0
            }
            if (room.terjawab[index]) return !0
            let users = global.DATABASE.data.users[m.sender]
...
${isSurrender ? '' : `+${room.winScore} XP cada respuesta es correcta`}
    `.trim()
        m.reply(caption, null, {
            contextInfo: {
                mentionedJid: this.parseMention(caption)
