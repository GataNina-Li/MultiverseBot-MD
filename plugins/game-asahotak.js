...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    let id = m.chat
    if (id in conn.asahotak) {
        conn.reply(m.chat, 'Todavía hay preguntas sin responder en este chat', conn.asahotak[id][0])
        throw false
    }
    let src = await (await fetch('https://raw.githubusercontent.com/BochilTeam/database/master/games/asahotak.json')).json()
    let json = src[Math.floor(Math.random() * src.length)]
...
Bonus: ${poin} XP
    ...
    conn.asahotak[id] = [
        ...
        json, poin,
        setTimeout(async () => {
            if (conn.asahotak[id]) await conn.sendBut(m.chat, `el tiempo se ha acabado!\nLa respuesta es ${json.jawaban}`, wm, 'Ejercicio mental', '.asahotak', conn.asahotak[id][0])
            delete conn.asahotak[id]
        }, timeout)
    ]
}
