let handler = async (m, { isOwner, text, isAdmin }) => {
  ...
  try {
    ...
    m.reply(`¡Hecho Desbanear! Bot activo en el chat ${await conn.getName(who) == undefined ? 'ini' : await conn.getName(who)}.`)
  } catch (e) {
    throw `el numero no existe en la base de datos!`
  }
}
handler.help = ['unban']
