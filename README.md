...
En caso que no tenga nada en el terminar puede usar este comando ```cd MultiverseBot-MD && rm -rf multiverse.data.json``` luego use ```node .``` Recuerde que este Bot no hace sesiones, deberá de tner una sesion previamente antes de hacer esta acción o la acción anterios
